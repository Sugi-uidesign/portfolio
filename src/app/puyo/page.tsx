...
export default function PuyoGame() {
  ...
  const handleStart = () => {
    dispatch({ type: 'START_GAME', nextPuyos: generateNextPuyos(3) });
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-4 py-8">
      <div className="w-full max-w-md flex items-center justify-between mb-4">
        <Link
          href="/"
          className="text-xs text-[#888888] hover:text-[#e8a4b8] transition-colors"
          style={{ fontFamily: '"Hiragino Mincho ProN", "Yu Mincho", serif' }}
        >
          ← Portfolio
        </Link>
        <h1 className="text-2xl font-black tracking-tight bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent select-none">
          PUYO PUYO
        </h1>
        <div className="w-16" />
      </div>

      <div className="flex gap-4 items-start" {...touchHandlers}>
